...
struct Parameters {
  ...
  float sim_th{1500.0f};  // Similarity threshold for the first step
  ...
  bool parse(const int argc, const char* const* const argv) {
    ...
    if (argc >= 7) window_size = std::atoi(argv[6]);
    if (argc >= 8) step_size = std::atoi(argv[7]);

    if (argc >= 9) gpu_device = std::atoi(argv[8]);
  ...
  void printParameters() const {
    ...
    std::cout << "     groud truth file: " << gt_filename << std::endl;

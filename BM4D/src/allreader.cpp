...
void AllReader::readSequence(const std::string& filename, std::vector<unsigned char>& volume, int& width, int& height, int& depth) {
...
void AllReader::read(const std::string& filename, std::vector<unsigned char>& volume, int& width, int& height, int& depth) {
    ...
    if (ext == ".txt" || ext == ".TXT") {
        ...
        std::cerr << "Unknown file extension: *" << ext << ". Available are tiff, avi, pgm (with sequence txt file)." << std::endl;
        exit(EXIT_FAILURE);
    }
}

...
__device__ void vec_add_so3( double * ,
			 double * ,
			 double * ,
			 int ) ;

__device__ void vec_add_scalar_so3( double ,
				double * ,
				double * ,
				int ) ;

__device__ void vec_mul_so3( double ,
			 double * ,
			 double * ,
			 int ) ;

__device__ void vec_mul_inplace_so3( double ,
				 double * ,
				 int ) ;

__device__ void vec_pt_mul_so3( double * ,
			    double * ,
			    double * ,
			    int ) ;

__device__ double vec_inner_so3( double * ,
			     double * ,
			     int ) ;

__device__ void transpose_so3( double * ,
			   double * ,
			   int ,
			   int ) ;

__device__ int totalCoeffs_so3( int ) ;

__device__ int numCoeffs_so3( int ,
			  int ) ;

__device__ int howMany_so3( int ,
			int ,
			int ) ;
__device__ int sampLoc_so3( int ,
			int ,
			int ) ;

__device__ int coefLoc_so3( int ,
			int ,
			int ) ;


__device__ int so3CoefLoc( int ,
		       int ,
		       int ,
		       int ) ;

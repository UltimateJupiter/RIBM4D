...
std::vector<uchar> BM4D::run_first_step() {
    ...
    std::cout << "Rotational Invariant Blockmatching took: " << blockmatching_rot.getSeconds() << std::endl;
    ...
    run_aggregation_rot(final_image, imshape, tshape, d_gathered4dstack, d_stacks, d_nstacks, d_group_weights, params, gather_stacks_sum, d_prop);
    aggregation_t.stop();
